...
class train_MIMIC_Dataset(Dataset):
    def __init__(self, txt_path='/data1/1shared/lijun/ecg/ECGRanking/data/train_LVH_modified.csv', ecg_path = '/hot_data/lijun/data/mimic-iv-ecg-diagnostic-electrocardiogram-matched-subset-1.0/'):
        """
        Args:
    ...
    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()
        result = 0
        hash_file_name = str(self.txt_path.iloc[idx, -1])
        diagnosis = self.txt_path.iloc[idx,-2]
...
class val_MIMIC_Dataset(Dataset):
    def __init__(self, txt_path='/data1/1shared/lijun/ecg/ECGRanking/data/test_final_machine_measurements.csv', ecg_path = '/hot_data/lijun/data/mimic-iv-ecg-diagnostic-electrocardiogram-matched-subset-1.0/'):
        """
        Args:
            labels_df (DataFrame): DataFrame containing the annotations.
            data_dir (str): Directory path containing the numpy data files.
